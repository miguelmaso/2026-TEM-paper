
\usepackage{geometry}
\geometry{
  a4paper,
  left=18mm,
  right = 18mm,
  top=20mm,
}

\usepackage{framed}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amsbsy}  % boldsymbol
\usepackage{bm,upgreek}
\usepackage{mleftright} % better spacing between brackets
\usepackage{nicefrac}
\usepackage{xfrac}
\usepackage{graphicx}
\usepackage{float}
\usepackage{anyfontsize} % Avoid font substitution warnings
\usepackage{csquotes}

\usepackage{subcaption}
\newsavebox{\imagebox}

\newcommand{\ra}[1]{\renewcommand{\arraystretch}{#1}}

\usepackage{siunitx}
\DeclareSIUnit\kelvin{^\circ K}

\newtheorem{prop}{Proposition}

\usepackage{caption}
\captionsetup[table]{
  font=normalsize, % Set caption font normal
  skip=5pt         % Reducing spacing between caption and table
}
\captionsetup[figure]{
  skip=2pt         % Reducing space between figure and caption
}
\captionsetup[subfigure]{
  skip=2pt         % Reducing space between figure and caption
}

\usepackage{array}
\usepackage{makecell}
\usepackage{colortbl}
\usepackage{booktabs}  % Table formatting: \toprule, \midrule, \bottomrule
\usepackage{tabularx}
\usepackage{multirow}
\newcolumntype{Y}{>{\raggedleft\arraybackslash}X} % see tabularx

\usepackage{tcolorbox}
\tcbuselibrary{skins}
\tcbset{enhanced,colback=blue!3,colframe=red!40,colbacktitle=blue!4,
coltitle=black,center title}

\biboptions{sort&compress}

% \usepackage{stmaryrd} % include jump condition

\usepackage[colorlinks,bookmarksopen,bookmarksnumbered,citecolor=red,urlcolor=red]{hyperref}
\pdfstringdefDisableCommands{%
  \def\corref#1{}%  Hyperref is unable to parse this commands to metadata
  \def\fnref#1{}%
}

\usepackage{color}
\newcommand{\Blue}[1]{{\color{blue}#1}}
\newcommand{\Red}[1]{{\color{red}#1}}
\newcommand{\Gray}[1]{{\color{gray}#1}}

\usepackage{tikz}
\usetikzlibrary{shapes,arrows,matrix,positioning}
\newcommand{\Cross}{\mathbin{\tikz [x=1.4ex,y=1.4ex,line width=.25ex] \draw (0.1,0.1) -- (0.9,0.9) (0.1,0.9) -- (0.9,0.1);}}

\usepackage{moreverb}
\usepackage{algorithm}
\usepackage{pseudocode}
\usepackage{algpseudocode} 

\graphicspath{{./figures/}}

% Matlab2Tikz
\usepackage{tikzscale,pgfplots}
\pgfplotsset{compat=1.13}
\newcommand\figurescale{1} 
\newlength\figH
\newlength\figW


% Definitions
\renewcommand{\vec}[1]{\ensuremath{\mbox{\boldmath $\mathrm{#1}$}}}

% Operators
\newcommand*{\Union}{\bigcup}
\newcommand{\vect}[1]{\boldsymbol{#1}}
\renewcommand{\d}[1][]{\ensuremath{\mathrm{d}#1}}     
\newcommand{\dx}{\ensuremath{\operatorname{dx}}}
\newcommand{\dX}{\ensuremath{\operatorname{dX}}}
\newcommand{\dV}{\ensuremath{\operatorname{dV}}}
\newcommand{\dA}{\ensuremath{\operatorname{dA}}}
\newcommand{\da}{\ensuremath{\operatorname{da}}}
\newcommand{\dv}{\ensuremath{\operatorname{dv}}}
\newcommand{\transp}{\ensuremath{^{\scriptstyle \mathrm{T}}}}
\newcommand{\h}{\ensuremath{^{\scriptstyle \mathrm{h}}}}
\renewcommand{\det}{\ensuremath{\mathrm{det}}}
\newcommand{\el}{\ensuremath{^{\scriptstyle \mathrm{e}}}}
\newcommand{\ti}{\ensuremath{_{\scriptstyle \mathrm{t}}}}
\renewcommand{\div}{\ensuremath{\operatorname{DIV}}}
\newcommand{\grad}{\ensuremath{\operatorname{grad}}}
\newcommand{\tr}{\ensuremath{\operatorname{tr}}}
\newcommand{\Grad}{\ensuremath{\operatorname{Grad}}}
\newcommand{\invT}{\ensuremath{^{\scriptstyle \mathrm{-T}}}}
\newcommand{\vol}{\ensuremath{\mathrm{vol}}}

\newcommand{\Aa}{\boldsymbol{A}_\alpha}
\newcommand{\A}{\mathcal{A}}
\newcommand{\B}{\mathcal{B}}
\newcommand{\C}{\boldsymbol{C}}
\newcommand{\Dvis}{\mathcal{D}}
\newcommand{\E}{\boldsymbol{E}}
\newcommand{\F}{\boldsymbol{F}}
\newcommand{\I}{\boldsymbol{I}}
\renewcommand{\H}{\boldsymbol{H}}  % ¶
\renewcommand{\P}{\boldsymbol{P}}  % ¶
\renewcommand{\S}{\boldsymbol{S}}  % §
\newcommand{\U}{\boldsymbol{U}}
\newcommand{\X}{\mathcal{X}}
\newcommand{\Eps}{\mathcal{E}}
\newcommand{\0}{\mathbf{0}}

% Figure placeholder box: \figplaceholder{height}{description of pending figure}
\newcommand{\figplaceholder}[2]{%
  \tcbox[width=\linewidth, height=#1, colback=gray!8, colframe=gray!55,
    valign=center, halign=center, sharp corners]{%
    \color{gray!50}\itshape #2}%
}

