...
\begin{equation} \label{eq:thero-mech-reference}
    \hat{\Psi}(\hat{\F}, \theta) = \hat{\Psi}_R(\hat{\F})
      ...
      \right)\ ,
\end{equation}
where $f(\theta_R)$ is a constant integral value that depends only on the specific heat function $g(\theta)$:
\begin{equation}
  f(\theta_R) = \int_0^{\theta_R} g(\vartheta) \left( \frac{\theta_R}{\vartheta} - 1 \right) \, d\vartheta .
\end{equation}



\begin{align*}
  g(\theta) = \left(\frac{\theta}{\theta_R}\right)^\gamma \quad &\rightarrow \quad
  f(\theta) = \left(\frac{\theta}{\theta_R}\right)^{\gamma+1} \\
%
  g(\theta) = \frac{1}{\xi_R}\left(\frac{\theta}{\theta_R}\right)^\gamma \quad &\rightarrow \quad
  f(\theta) = \frac{1-\left(\frac{\theta}{\theta_M}\right)^{\gamma+1}}{-1\left(\frac{\theta_R}{\theta_M}\right)^{\gamma+1}} \\
%
  g(\theta) = \ \dots \hspace{1em} \quad &\rightarrow \quad
  f(\theta) = (1-\delta)\left(\frac{\theta}{\theta_R}\right)^\gamma + \delta \\[3pt]
%
  g(\theta) = \frac{\theta}{\theta_R}\sin\left(\frac{2\pi\theta}{\theta_M}\right) \quad &\rightarrow \quad
  f(\theta) \propto 1 -\theta + \sin\left(\frac{2\pi\theta}{\theta_M}\right) \\[5pt]
%
  g(\theta) \propto \theta^2 \quad &\rightarrow \quad
  f(\theta) = a(\theta - \theta_R)^3 + b(\theta - \theta_R)^2 + c(\theta - \theta_R) + 1 \\[5pt]
%
  g(\theta) = \frac{1}{\theta\sigma2\pi}\exp\left(-\frac{(\ln\theta-\mu)^2}{2\sigma^2}\right) \quad &\rightarrow \quad
  f(\theta) \propto 1-CDF(\theta)
\end{align*}
...
\begin{equation*}
  \hat\Psi(\F,\theta) = \hat{h}(\theta)\hat\Psi_R(\F)
\end{equation*}


\begin{equation*}
  \Psi(\F,\theta) = 
    \bar{f}(\theta) \bar\Psi_R(J) \, + \,
    \hat{f}_{\infty}(\theta) \hat\Psi_{R,\infty}(\F) \, + \,
    \sum_{\alpha} \hat{f}_\alpha(\theta) \hat\Psi_\alpha(\F) \, + \,
    \hat{f}_E(\theta) \hat\Psi_E(\F,\E_0)
\end{equation*}

\begin{equation*}
  c_v = -\theta \frac{\partial^2\Psi}{\partial\theta^2} \ ; \quad c_{v,i} = g_i \Psi_i
\end{equation*}

\begin{equation*}
  f_i = \frac{1}{\xi_{R,i}}
    \int_0^\theta g_i(\vartheta) \left( \frac{\theta}{\vartheta} - 1 \right) d\vartheta
\end{equation*}

\begin{align*}
  f(\theta_R) &= 1 \\
  f'(0) &= 0 \\
  f(\theta) &> 0 \quad \forall \ \theta \in \mathbb{R}^+ \quad \text{or alternatively,} \ [0,\theta_M) \\
  g(1) &= \dots
\end{align*}
...
\begin{equation}
  \hat\Psi(\F,\theta) = \hat\Psi_R(\F) \left( 1 -
    \frac{f(\theta) - f(\theta_R)}{f(\theta_M) - f(\theta_R)}
  \right)\ ,
\end{equation}
which allows to find a relation between $\hat\Psi_R$ and $\hat\eta_R$. For convenience, terms in expression (9) can be gathered into a single function depending on the temperature
\begin{equation}
  h(\theta) = \frac{f(\theta_R) - f(\theta)}{f(\theta_M) - f(\theta_R)} \ ,\quad 
  \hat\Psi(\F,\theta) = \hat\Psi_R(\F) (1 + h(\theta)) .
\end{equation}



\section*{Concrete functions and thermodinamical constraints}

The function $g$ is not specified \emph{a priori} and must be determined...
\begin{equation}
  g(\theta) = \frac{\theta}{\theta_R}\sin\left(\frac{2\pi\theta}{\theta_M}\right)
\end{equation}

\dots

\begin{equation*}
  \hat\P(\F,\theta) = \pder{\hat\Psi_R}{\F}(1 + h(\theta))
\end{equation*}

\begin{equation*}
  \hat\eta(\F,\theta) = -\hat\Psi_R h'(\theta)
\end{equation*}

\begin{equation*}
  \hat c_v(\F,\theta) = -\theta \hat\Psi_R h''(\theta)
\end{equation*}
...
\begin{equation*}
  G(\theta) = -\frac{1}{2\pi}\frac{\theta_M}{\theta_R}\cos\left(\frac{2\pi\theta}{\theta_M}\right)
\end{equation*}

\begin{equation*}
  f(\theta) = -\frac{1}{4\pi^2}\frac{\theta_M^2}{\theta_R}\sin\left(\frac{2\pi\theta}{\theta_M}\right)
\end{equation*}


constraints on $h$:
\begin{itemize}
  \item $h > 0$ in $[0,\theta_M)$
  \item $h'(0)=0$
  \item $h(\theta_R) = 1$
  \item $h(\theta_M) = 0$
\end{itemize}
