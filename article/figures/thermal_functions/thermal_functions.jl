...
funcs_f = map(law -> law()[1], laws)
funcs_η = map(law -> law()[2], laws)
funcs_G = map(df -> (θ) -> df(θ)/df(1.0), funcs_η)
...
for i in 1:N
  plot!(0:0.01:2, funcs_f[i]; label=labels[i], style=styles[i])
end
scatter!([1],[1], color=:black, label=nothing)
savefig(abspath(dirname(@__FILE__), "..", stem(@__FILE__) * "_Psi_" * string(N) * ".pdf"))
display(p);


p = plot(; xlabel=L"\theta / \theta_R", ylabel=L"\eta / \eta_R", extra_kwargs)
for i in 1:N
  plot!(0:0.01:2, funcs_G[i]; label=labels[i], style=styles[i], ylim=[-0.5, Inf])
end
scatter!([1],[1], color=:black, label=nothing)
savefig(abspath(dirname(@__FILE__), "..", stem(@__FILE__) * "_eta_" * string(N) * ".pdf"))
display(p);
