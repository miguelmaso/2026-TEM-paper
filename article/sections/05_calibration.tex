...
\paragraph{Parameter uncertainty and conficence boundaries}
...
The previous definition of the kinematics in terms of a local state $\vect{x}_i$ and scalar energy-conjugate observable $y_i$ ensures that the multi-objective cost functional \eqref{eqn:total_loss} can be evaluated independently of each step. The sequence of these calibration steps is summarized in Table \ref{tab:calibration_steps}, tracking the progression from purely thermal properties to fully coupled thermo-electro-viscoelastic behavior, concluding with a multi-field validation step.
...
2a & Quasi-static \cite{Liao_Mokarram2020} & $\lambda$ & $P_{11}$ & $\mu_1,\ \alpha_1,\ \mu_2,\ \alpha_2$ \\ \addlinespace
2b & Quasi-static \cite{Liao_Mokarram2020} & $\lambda$ & $P_{11}$ & $C_{10},\ C_{20},\ C_{30}$ \\ \addlinespace
