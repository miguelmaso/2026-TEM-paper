...
The previous definition of the kinematics in terms of a local state $\vect{x}_i$ and scalar energy-conjugate observable $y_i$ ensures that the multi-objective cost functional \eqref{eqn:total_loss} can be evaluated independently of each step. The sequence of these calibration steps is summarized in Table \ref{tab:calibration_steps}, from purely thermal properties to fully coupled thermo-electro-viscoelastic behavior, concluding with a multi-field validation step.
