...
The unknown fields $\{\vect{v},\vect{\phi},\varphi,\vect{D}_0\theta\}$ in the semi-discrete weak forms $\left(\mathcal{W}_{\vect{v}}\right)_{\text{algo}}$, $\left(\mathcal{W}_{\vect{\phi}}\right)_{\text{algo}}$, $\left(\mathcal{W}_{\phi}\right)_{\text{algo}}$,  $\left(\mathcal{W}_{\vect{D}_0}\right)_{\text{algo}}$and $\left(\mathcal{W}_{{\theta}}\right)_{\text{algo}}$ in \eqref{eqn:weak forms for proposed time integrator} are discretised employing the following functional spaces $\mathbb{V}^{\vect{\phi}^h}\times\mathbb{V}^{\vect{\phi}^h}\times\mathbb{V}^{{\varphi}^h}\times\mathbb{V}^{\vect{D}_0^h}\times\mathbb{V}^{{\theta}^h}$ defined as
%
\begin{equation}\label{eqn:functional spaces for discretisation}
\begin{aligned}
\mathbb{V}^{\vect{\phi}^h}& = \{\vect{\phi}\in \mathbb{V}^{\vect{\phi}};\,\,\,\,\left.\vect{\phi}^h\right\vert_{\mathcal{B}_0^e} = \sum_{a=1}^{n_{\text{node}}^{\vect{\phi}}}N^{\vect{\phi}}_a\vect{\phi}_a\};&\qquad
%
%
%
\mathbb{V}^{\varphi^h}& = \{\varphi\in \mathbb{V}^{\varphi};\,\,\,\,\left.\varphi^h\right\vert_{\mathcal{B}_0^e} = \sum_{a=1}^{n_{\text{node}}^{\varphi}}N^{\varphi}_a\varphi_{a}\};\\
%
\mathbb{V}^{\vect{D}_0^h}& = \{\vect{D}_0\in \mathbb{V}^{\vect{D}_0};\,\,\,\,\left.\vect{D}_0^h\right\vert_{\mathcal{B}_0^e} = \sum_{a=1}^{n_{\text{node}}^{\vect{D}_0}}N^{\vect{D}_0}_a{\vect{D}_0}_a\};&\qquad
%
%
%
...
Similarly, following a Bubnov-Galerkin approach, the functional spaces for the test functions $\{\vect{w}_{\vect{v}},\vect{w}_{\vect{\phi}},w_{\varphi},\vect{w}_{\vect{D}_0},w_{\theta}\}\in \mathbb{V}^{\vect{\phi}^h}_0\times\mathbb{V}^{\vect{\phi}^h}_0\times\mathbb{V}^{{\varphi}^h}_0\times\mathbb{V}^{\vect{D}_0^h}\times \mathbb{V}_0^{\theta^h}$ are defined as
%
...
%
\left(\mathcal{W}_{\vect{D}_0}\right)_{\text{algo}}&=\sum_{e=1}^N{\vect{w}_{\vect{D}_0}}_a\cdot\vect{R}_{a,e}^{\vect{D}_0};&\quad
%
%
...
\begin{aligned}
\vect{R}_{a,e}^{\vect{\phi}} & = \int_{\mathcal{B}^e_0}\rho_0N^a_{\vect{\phi}}\left(2\frac{\Delta\vect{\phi}}{\Delta t^2} - 2\frac{\vect{v}_n}{\Delta t}\right)\,dV + \int_{\mathcal{B}_0^e}\left(\vect{F}_{n+1/2}\vect{S}_{\text{algo}}\right)\vect{\nabla}_0N^{\vect{\phi}}_a\,dV + \int_{\mathcal{B}_0^e}N^{\vect{\phi}}_{a}\vect{f}_{0_{n+1/2}}\,dV;\\
%
%
R^{\varphi}_{a,e}&=  \int_{\mathcal{B}_0}\vect{D}_{0_{n+1/2}}\cdot\vect{\nabla}_0N_a^{\varphi}\,dV+  \int_{\mathcal{B}_0}\rho_{0_{n+1/2}}^eN_a^{\varphi}\,dV;\\
%
\left(\mathcal{W}_{\vect{D}_0}\right)_{\text{algo}}&=\int_{\mathcal{B}_0} \left({\vect{E}_0}_{\text{algo}}+ \vect{\nabla}_0\varphi_{n+1/2}\right)N_a^{\vect{D}_0}\,dV;\\
%
%
R_{a,e}^{\theta} & =\int_{\mathcal{B}_0}\frac{\Delta\left(\theta\eta\right)}{\Delta t}N^{\theta}_a\,dV - \int_{\mathcal{B}_0}\frac{\Delta \theta}{\Delta t}\eta_{\text{algo}}N^{\theta}_a\,dV - \int_{\mathcal{B}_0}\vect{Q}_{n+1/2}\cdot\vect{\nabla}_0N^{\theta}_a\,dV - \int_{\mathcal{B}_0}{R_{\theta}}_{n+1/2}N^{\theta}_a\,dV.
%
\end{aligned}
\end{equation}
%
