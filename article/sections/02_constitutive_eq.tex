...
\end{equation}
%
The relaxation time is represented by the parameter $\tau_\alpha$, that determines the rate of dissipation of the viscous stresses. The last term in \eqref{eqn:evol_law} represent a possible a possible volumetric contribution,  guaranteeing the incompressibility. Using Eqs. \eqref{eqn:visco_Je_t} \eqref{eq23} along with above Eq. \eqref{eqn:evol_law}, the value of the scalar parameter $\lambda_\alpha$ (i.e. Lagrange multiplier) can be obtained as
%
\begin{equation}
	\label{eq67}
...
For completeness, by the use of Eqs.~\eqref{eqn:evol_law} and \eqref{eq67}, the viscous dissipation can finally take the form of
%
\begin{equation}
	\label{eq68}
	\begin{aligned}
...
\end{equation}
%
and the positive value of the numerator is a consequence of the Cauchy-Schwarz inequality. An alternative derivation of the evolution law \eqref{eq66} and the value of the Lagrange multiplier \eqref{eq67} can be found in the Appendices of \cite{Liu2025}.
...
\end{equation}
%
The evolution law for $\boldsymbol{S}^{'}_{\mathrm{e}_{\alpha}}$ embodied by Eq.~\eqref{eqn:evol_law} in the compressible case, can now be adjusted in order to comply with the incompressibility constraint Eq.~\eqref{eqn:visco_Je_t}. This can be carried out via the use of a scalar parameter $\lambda_\alpha$ (i.e. Lagrange multiplier) to give
%
\begin{equation}
	\label{eq66}
...
\end{equation}
%
where the last term on the right hand side represent a possible volumetric stress contribution. Using Eqs.~\eqref{eqn:visco_Je_t} and \eqref{eq65.2} along with above Eq.~\eqref{eq66}, the value of $\lambda_\alpha$ can be obtained after simple algebra as
%
\begin{equation}
	\label{eq67}
...
For the sake of computational implementation, Eq.~\eqref{eq31} would need to be integrated in time so as to update the state variables $\boldsymbol{U}_{\mathrm{v}_\alpha}$ from time step $n$ to time step $n+1$. However, the direct use of this equation is highly cumbersome, and thus it is preferable to transform the left-hand side of Eq.~\eqref{eqn:evol_law} into an incremental form, which when using a classical (first order) backward Euler scheme renders
%
\begin{equation}
	\label{eq34}
...
%Note that the above constraint Eq.~\eqref{eqn:visco_Je_t} implies that the relationship \eqref{eq23} re-written as
%%
%\begin{equation}
%	\label{fakeeq64}
