...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{figures/TheMappingContinuum.pdf}
	\end{center}
...
\subsection{Governing equations: conservation of linear momentum and angular momentum}\label{sec:conservation of linear momentum}
...
\begin{aligned}
\rho_0\dot{\vect{v}} - \text{DIV}\left(\vect{P}\right) - \vect{f}_0& = \vect{0};&\qquad &\text{in}\,\,\mathcal{B}_0\times \left[0,T\right];\\
%
\left(\vect{F}\vect{S}\right)\vect{N}& = \vect{t}_0;&\qquad&\text{on}\,\,\partial_{\boldsymbol{t}}\mathcal{B}_0\times \left[0,T\right];\\
%
\vect{\phi} &= \bar{\vect{\phi}};&\qquad & \text{on}\,\,\partial_{\vect{\phi}}\mathcal{B}_0\times \left[0,T\right];\\
%
\left.\dot{\vect{\phi}}\right\vert_{t=0}&=\dot{\bar{\vect{\phi}}};&\qquad &\text{in}\,\,\mathcal{B}_0;\\
%#
\left.{\vect{\phi}}\right\vert_{t=0}&=\bar{\vect{\phi}};&\qquad
&\text{in}\,\,\mathcal{B}_0,
%
\end{aligned}
\end{equation}
...
\subsection{Governing equations in non-linear thermo-mechanics: Gauss's and Faraday's laws}\label{sec:Gauss and Faraday}
...
\begin{aligned}
\text{DIV}\vect{D}_0 - \rho_0^e&=0;&\qquad &\text{in}\,\mathcal{B}_0\times \left[0,T\right]\\
%
%
...
\subsection{Governing equations in non-linear thermo-mechanics: conservation of energy}\label{sec:conservation of energy}

The balance of energy equation can be written as
%
\begin{equation}\label{eqn:local form energy}
\begin{aligned}
\theta\dot{\eta}  - \mathcal{D} + \text{DIV}\vect{Q} - R_{\theta} & = 0;&\qquad& \text{in}\,\,\mathcal{B}_0\times \left[0,T\right];\\
%
...
%%#
\left.{\theta}\right\vert_{t=0}&={\bar{\theta}};&\qquad& \text{in}\,\,\mathcal{B}_0,
\end{aligned}
\end{equation}
%
...
With the aim of simplifying the time discretisation of \eqref{eqn:local form energy}, the authors in \cite{XXX} proposed an alternative but equivalent re-expression (at the continuum level) to that in \eqref{eqn:local form energy} as
%
\begin{equation}\label{eqn:local form energy our format}
\begin{aligned}
\frac{d}{dt}\left(\theta\eta\right) - \dot{\theta}\eta -\mathcal{D} + \text{DIV}\vect{Q} - R_{\theta} & = 0;&\qquad& \text{in}\,\,\mathcal{B}_0\times \left[0,T\right];\\
%
\vect{Q}\cdot\vect{N}& = -Q_{\theta};&\qquad&\text{on}\,\, \partial_{Q}\mathcal{B}_0\times \left[0,T\right];\\
%
\theta & = \bar{\theta};&\qquad&\text{on}\,\,\partial_{\theta}\mathcal{B}_0\times \left[0,T\right];\\
...
In this work we will advocate for \eqref{eqn:local form energy our format}.
