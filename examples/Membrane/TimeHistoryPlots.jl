...
θavg = m.θavg .- 273.15
θmean = moving_average(θavg, 50)
Dmean = moving_average(m.Dvis, 50)
p11 = plot(m.time, θavg,         lw=1, lcolor=1, margins=3mm, label="", xformatter=_->"", ylabel=L"\theta\ [ºC]")
plot!(     m.time, θmean,        lw=3, lcolor=1, margins=3mm, label="")
p12 = plot(m.time, m.λ,          lw=1, lcolor=2, margins=3mm, label="", xformatter=_->"", ylabel=L"\lambda\ [-]")
plot!(     m.time[i_max], λ_max, lw=3, lcolor=2, margins=3mm, label="")
p13 = plot(m.time, m.Dvis.*1e5,  lw=1, lcolor=3, margins=3mm, label="", xlabel="Time [s]", ylabel=L"\mathcal{D}_{\rm{int}}\ [W\times10^{-5}]")
plot!(     m.time, Dmean*1e5,    lw=3, lcolor=3, margins=3mm, label="")
plot!([9,10,10,9,9],[0,0,4,4,0], lw=1, lcolor=:black, label="")
plot!(m.time[9000:end], m.Dvis[9000:end], lw=1, lcolor=3, label="", xformatter=_->"", yformatter=_->"", inset=(1, bbox(0.6,0.0,0.3,0.6)), subplot=2, framestyle=:box)
