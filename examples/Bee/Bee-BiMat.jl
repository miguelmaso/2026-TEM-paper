...
jac_u(Λ) = (u, du, v) -> ∫(∇(v)' ⊙ ((∂∂Ψ₁∂FF ∘ (F∘∇(u)', Eh)) ⊙ ∇(du)'))dΩ₁ +
                         ∫(∇(v)' ⊙ ((∂∂Ψ₂∂FF ∘ (F∘∇(u)'    )) ⊙ ∇(du)'))dΩ₂

jac_φ(Λ) = (φ, dφ, vφ) -> ∫(∇(vφ)' ⋅ ((∂∂Ψ₁∂EE ∘ (Fh, E∘∇(φ))) ⋅ ∇(dφ)))dΩ₁
...
createpvd(outpath) do pvd
  ...
  try
    while time < t_end
      ...
      op_φ = FEOperator(res_φ(time), jac_φ(time), Uφ, Vφ)
      solve!(φh⁺, solver, op_φ)

      op_u = FEOperator(res_u(time), jac_u(time), Uu, Vu)
      solve!(uh⁺, solver, op_u)
