...
set_7_dielec = load_data(abspath(@__DIR__, "data/set 7 dielectric.csv"), DielectricTest)


## Calorimetry test
p1 = plot(xlabel="Temperature [K]", ylabel="Specific heat [kJ/m³/K]", legend=:topleft)
for v ∈ [40, 30, 20, 10]
  exp_1 = getfirst(r -> r.v ≈ v, set_1_heat)
  label = @sprintf("%2.0f K/min", v)
  step = 3
  scatter!(exp_1.θ[begin:step:end], exp_1.cv[begin:step:end] ./ 1e3; label)
end
display(p1);
...
for λ ∈ [1.25, 2, 3, 4]
  ...
  label = @sprintf("20ºC, %3.0f%%", (λ-1)*100)
