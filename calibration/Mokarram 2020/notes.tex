...
These notes describe the implementation of the model proposed by \cite{mokarram2020}. The purpose of these notes is to review the correctness of the current implementation, as well as to detect any discrepancies with respect to the data provided by \cite{mokarram2020}.
...
For simplicity and for strict comparison with the model presented by \cite{mokarram2020}, we consider the uniaxial case where the deformation gradient $\F$ reduces to a scalar stretch $\lambda$:
\begin{equation}
    \F = \begin{bmatrix}
        \lambda & 0 & 0 \\
        0 & \lambda^{-1/2} & 0 \\
...
Assuming incompressibility and uniaxial stretch, the rate of change of the viscous stretch $\dot{\lambda}_v$ is defined by the flow law:
\begin{equation}
    \dot{\lambda}_v = \frac{2\lambda}{3\eta} P_{e}(\lambda_e)
\end{equation}
Where the effective viscosity is $\eta = \tau \mu$.
To obtain $\lambda_{v,n+1}$, an implicit Backward Euler has been chosen, where the residual and Jacobian are defined as follows:
\begin{equation}
...
The following tables summarize the parameters used in the calibration of the model by \cite{mokarram2020} for the uniaxial case. The parameters are obtained from fitting the experimental data provided in their paper, and they are used to define the hyperelastic response (table (\ref{tab:carroll_params})), the viscoelastic branches (tables (\ref{tab:yeoh_viscous}) and (\ref{tab:neoh_viscous})), and the thermal scaling functions (table (\ref{tab:thermal_params})).
...
The implemented model is compared with the experimental data provided by \cite{mokarram2020} for the uniaxial case in figures (\ref{fig:relaxation_temp20}) and (\ref{fig:stretch300_temp20}). The long term response shows good agreement between the implemented model and the results reported by \cite{mokarram2020}, see figure (\ref{fig:relaxation_temp20}). Secondly, from the comparison made in figure (\ref{fig:stretch300_temp20}) for a one-cycle loading-unloading at constant maximum stretch, reference temperature and different loading velocities, it is clear that the implemented model underestimates the stress response and needs validation.


\begin{figure}[ht]
    \centering
    \includegraphics[width=0.45\textwidth]{relaxation_temp20.png}
    \includegraphics[width=0.45\textwidth]{paper_relaxation_temp20.png}
    \caption{Comparison of the model against experimental data. Left: implemented model. Right: calibration reported by \cite{mokarram2020}.}
    \label{fig:relaxation_temp20}
\end{figure}
...
\bibitem{mokarram2020}
  Zisheng Liao, Mokarram Hossain, Xiaohu Yao, Markus Mehnert, Paul Steinmann,
  \textit{On thermo-viscoelastic experimental characterization and numerical modelling of VHB polymer,},
  International Journal of Non-Linear Mechanics, Volume 118 (2020),
  103263, ISSN 0020-7462,
  doi.org/10.1016/j.ijnonlinmec.2019.103263
